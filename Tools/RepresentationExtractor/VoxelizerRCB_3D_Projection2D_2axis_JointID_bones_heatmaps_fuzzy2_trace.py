...
class VoxelizerRCB_3D_Projection2D_2axis_JointID_bones_heatmaps_fuzzy2_trace(MirroirableVoxelization):
    def name(self) -> str:
        return "Project2Axis_fuzzy2_Trace"
    def finalSizeBox(self) -> Tuple:
        ...
        return self.sizeBox[0]*2, self.sizeBox[1], 1 + len(self.jointSelected) + self.morphoLogy.getNbBones(), 1

    def __init__(self, sizeBoxInit, thresholdToleranceCuDi, threshCurviDist, jointSelected, morphology: Morphology):
        super(VoxelizerRCB_3D_Projection2D_2axis_JointID_bones_heatmaps_fuzzy2_trace, self).__init__(sizeBoxInit,
    ...
    def getHeatMapForBone(self, bonept1, bonept2):
        dimX = self.finalSizeBox()[0]
        dimY = self.finalSizeBox()[1]
        heatMap = np.zeros((dimX, dimY))
        stdDev, ck = 2, 1
        for i in range(dimX):
            for j in range(dimY):
                heatMap[i][j] = math.exp(-self.getDistToBoneXY(bonept1, bonept2, i, j) ** 2 / (2 * stdDev) * ck)
        return heatMap
        ...
        while postureID < len(data):
            ...
            while cudi < self.threshCurviDist and postureID < len(data):
                ...
                for i in range(len(self.jointSelected)):
                    ...
                    posM1 = postureNormalized.joints[self.jointSelected[i]].position
                    pos = postureNormalizedPlus1.joints[self.jointSelected[i]].position
                    points = self.getNewListOfPointBetween(posM1,pos,1)
                    for idPoint, point in enumerate(points):
                        if idPoint==0 or (int(point[0])!=int(points[idPoint-1][0]) or
                                          int(point[1])!=int(points[idPoint-1][1]) or
                                          int(point[2])!=int(points[idPoint-1][2])):
                            box[int(point[0]), int(point[1]), 0, 0] = repeat + 1
                    for idPoint, point in enumerate(points):
                        if idPoint==0 or (int(point[0])!=int(points[idPoint-1][0]) or
                                          int(point[1])!=int(points[idPoint-1][1]) or
                                          int(point[2])!=int(points[idPoint-1][2])):
                            box[self.sizeBox[0]+int(point[2]), int(point[1]), 0, 0] = repeat + 1
                ...
                for z in range(posZMin+1, posZMax+1):
                    for y in range(posYMin+1, posYMax+1):
                        if z < 0 or z >= self.sizeBox[0] or y < 0 or y >= self.sizeBox[1]:
                            continue
                        box[self.sizeBox[0] + z, y, 1 + j, 0] = math.exp(-((z - pos[2]) ** 2 + (y - pos[1]) ** 2) / (2 * stdDev) * ck)
