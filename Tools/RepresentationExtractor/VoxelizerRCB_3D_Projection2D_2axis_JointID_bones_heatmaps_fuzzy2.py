...
class VoxelizerRCB_3D_Projection2D_2axis_JointID_bones_heatmaps_fuzzy2(MirroirableVoxelization):

    def name(self) -> str:
        return "Project2Axis_fuzzy2"
    ...
    def __init__(self, sizeBoxInit, thresholdToleranceCuDi, threshCurviDist, jointSelected, morphology: Morphology):
        super(VoxelizerRCB_3D_Projection2D_2axis_JointID_bones_heatmaps_fuzzy2, self).__init__(sizeBoxInit,
    ...
    def extractRepresentation(self, data: List[Posture]) -> Tuple[List[np.array], List[int]]:
        ...
        while postureID < len(data):
            ...
            while cudi < self.threshCurviDist and postureID < len(data):
                ...
                postureID += 1
                repeat += 1
            box[:, :, 0, 0] /= repeat # for the Trace map, normalize the values
            repeats.append(repeat + repeatb4)
            repeatb4 = 0

            stdDev = 1.3
            dist = 3 # before and after X/y of min/max coordinates, => bounding box with margin of "dist"
            ck = 1
            ...
            for j in range(len(self.jointSelected)):
                ...
                distance = 2.5 #around each joint
