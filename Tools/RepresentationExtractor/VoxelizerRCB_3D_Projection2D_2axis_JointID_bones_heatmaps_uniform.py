...
class VoxelizerRCB_3D_Projection2D_2axis_JointID_bones_heatmaps_uniform(MirroirableVoxelization):
    def name(self) -> str:
        return "Project2Axis_fullHeatmaps"
    ...
    def __init__(self, sizeBoxInit, thresholdToleranceCuDi, threshCurviDist, jointSelected, morphology: Morphology):
        super(VoxelizerRCB_3D_Projection2D_2axis_JointID_bones_heatmaps_uniform, self).__init__(sizeBoxInit,
    ...
    def extractRepresentation(self, data: List[Posture]) -> Tuple[List[np.array], List[int]]:
        ...
        while postureID < len(data):
            ...
            # jointsDisplacmentPerJoint = [[] for i in range(len(self.jointSelected))]
            ...
            for iBone, bones in enumerate(self.morphoLogy.getBones()):
                ...
                bornMinX = 0
                bornMaxX = self.sizeBox[0]

                bornMinY = 0
                bornMaxY = self.sizeBox[1]

                bornMinZ = 0
                bornMaxZ = self.sizeBox[2]
            ...
            for j in range(len(self.jointSelected)):
                ...
                # distance = 1.5
                posXMin = 0
                posXMax = self.sizeBox[0]
                posYMin = 0
                posYMax = self.sizeBox[1]
                posZMin = 0
                posZMax = self.sizeBox[2]

                for x in range(posXMin, posXMax):
                    for y in range(posYMin, posYMax):
                        if x < 0 or x >= self.sizeBox[0] or y < 0 or y >= self.sizeBox[1]:
                            continue
                        box[x, y, 1 + j, 0] = math.exp(-((x - pos[0]) ** 2 + (y - pos[1]) ** 2) / (2 * stdDev) * ck)
                for z in range(posZMin, posZMax):
                    for y in range(posYMin, posYMax):
                        if z < 0 or z >= self.sizeBox[0] or y < 0 or y >= self.sizeBox[1]:
                            continue
                        box[self.sizeBox[0] + z, y, 1 + j, 0] = math.exp(
                            -((z - pos[2]) ** 2 + (y - pos[1]) ** 2) / (2 * stdDev) * ck)
