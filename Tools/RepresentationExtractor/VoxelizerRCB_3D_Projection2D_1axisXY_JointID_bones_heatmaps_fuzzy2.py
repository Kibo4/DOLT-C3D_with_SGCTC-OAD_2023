...
class MirroirableVoxelization(VoxelizerHandler, ABC):
    transposition = None
    ...
    def getTranspositionFeaturesForMirroredVoxelization(self):
        ...
        for i in range(len(self.jointSelected)):
            idJointMirror = MorphologyGetter.getMirrorMember(self.morphoLogy.deviceName, self.jointSelected[i])
            if idJointMirror not in self.jointSelected:
                raise Exception("joint " + str(idJointMirror) + "mirror of ", i, " not in jointSelected, cannot mirror")

            # print("mirror of ", self.jointSelected[i], "is", idJointMirror)

            positionInFeature = 1 + i
            positionWhereToTranspose = 1 + self.jointSelected.index(idJointMirror)
            # print("positionInFeature", positionInFeature, "positionWhereToTranspose", positionWhereToTranspose)

            if positionInFeature not in transposition.values() \
                    and positionWhereToTranspose not in transposition \
                    and positionInFeature != positionWhereToTranspose:
                transposition[positionInFeature] = positionWhereToTranspose
        dictMapbones = self.morphoLogy.getUniqueMappingForMirrorBones()
        for id in dictMapbones:
            positionInFeature = 1 + len(self.jointSelected) + id
            positionWhereToTranspose = 1 + len(self.jointSelected) + dictMapbones[id]
            if positionInFeature not in transposition.values() and positionWhereToTranspose not in transposition:
                transposition[positionInFeature] = positionWhereToTranspose
        self.transposition = transposition
        return self.transposition


class VoxelizerRCB_3D_Projection2D_1axisXY_JointID_bones_heatmaps_fuzzy2(MirroirableVoxelization):

    def name(self) -> str:
        return "Project1AxisXY_fuzzy2"

    def finalSizeBox(self) -> Tuple:
        # print(1+len(self.jointSelected)+self.morphoLogy.getNbBones(),len(self.jointSelected),self.morphoLogy.getNbBones())
    ...
    def __init__(self, sizeBoxInit, thresholdToleranceCuDi, threshCurviDist, jointSelected, morphology: Morphology):
        super(VoxelizerRCB_3D_Projection2D_1axisXY_JointID_bones_heatmaps_fuzzy2, self).__init__(sizeBoxInit,
    ...
    def extractRepresentation(self, data: List[Posture]) -> Tuple[List[np.array], List[int]]:
        ...
        while postureID < len(data):
            ...
            for iBone, bones in enumerate(self.morphoLogy.getBones()):
                ...
                # posJoint1ZY = np.array([posJoint1[2], posJoint1[1]])
                # posJoint2ZY = np.array([posJoint2[2], posJoint2[1]])
                bornMinX = int(min(posJoint1[0],posJoint2[0]) - dist)
                bornMaxX = int(max(posJoint1[0],posJoint2[0]) + dist)
                bornMinX = int(max(0, bornMinX))
                bornMaxX = int(min(self.sizeBox[0], bornMaxX))
                ...
                for y in range(bornMinY, bornMaxY):
                    for x in range(bornMinX, bornMaxX):
                        if y>=self.sizeBox[1] or x>=self.sizeBox[0]:
                            continue
                        boxXY = box[x, y]
                        boxXY[1 + len(self.jointSelected) + iBone, 0] = math.exp(
                            -self.getDistToBoneXY(posJoint1, posJoint2, x, y) ** 2 / (2 * stdDev) * ck)


            for j in range(len(self.jointSelected)):
                pos = postureNormalized.joints[self.jointSelected[j]].position
                distance = 2.5
                posXMin = int(pos[0] - distance)
                posXMax = int(pos[0] + distance)
                posYMin = int(pos[1] - distance)
                posYMax = int(pos[1] + distance)

                for x in range(posXMin+1, posXMax+1):
                    for y in range(posYMin+1, posYMax+1):
                        if x < 0 or x >= self.sizeBox[0] or y < 0 or y >= self.sizeBox[1]:
                            continue
                        box[x, y, 1 + j, 0] = math.exp(-((x - pos[0]) ** 2 + (y - pos[1]) ** 2) / (2 * stdDev) * ck)
