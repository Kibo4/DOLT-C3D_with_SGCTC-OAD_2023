...
class OLTC3D_doubleStream(tf.keras.Model):
    """
    ...
    (batch_size, time, width*2, height, channels)
    the two viewpoints which will be used in different streams are concatenated in the width dimension.
    it must be the same width for both views.
    ...
    def __init__(self, nbClass, dropoutVal: float = 0.2, boxSize: Tuple[int, int, int] = (30, 15, 32),
        ...
        :param boxSize: the size of the input box, (width*2, height,channels), the width consider the two views
        ...
        :param doGlobalAveragePooling: if True, the global average pooling is applied at the end of the
                    convolutional layers, allows to remove the width and height dimensions
        """
        super(OLTC3D_doubleStream, self).__init__()
        ...
        if self.doMaxPoolSpatial:
            ...
            self.maxPoolSpatialLayerSame = [MaxPool3D(pool_size=self.poolSize, strides=self.poolStrides,
                                                      padding="same") for _ in range(len(dilatationsRates)* numberOfBlock)]
        ...
        for id, dilatRate in enumerate(self.dilatationsRates):
            ...
            if id == 0:
                conv = Conv3D(filters=self.nbFeatureMaps[id + 1], kernel_size=self.kernelSize1, strides=1,
                              ...
                              input_shape=[None, self.boxSize[0]//2, self.boxSize[1], self.boxSize[2]],
    ...
    def call(self, x, training=True, **kwargs):
        ...
        maxlengthX = tf.shape(x)[2]  # = boxDim[0]*2
        maxlengthX = maxlengthX // 2  # = boxDim[0]
        maxlengthY = tf.shape(x)[3]  # = boxDim[1]

        x = tf.cast(x, dtype=tf.float32)

        x = tf.squeeze(x, axis=-1)
        xy = x[:, :, :maxlengthX, :, :]  # first half is XY viewPoint
        zy = x[:, :, maxlengthX:, :, :]  # second half is ZY viewPoint
        ...
        for idLayer, layer in enumerate(self.layersConv):
            ...
            topadX = maxlengthX + topadX - tf.shape(xy)[2]
            topadY = maxlengthY + topadY - tf.shape(xy)[3]
            ...
            xy = tf.pad(xy, paddings=paddings, mode="CONSTANT",
                        constant_values=0.)  # -> [batch, seq(pad), x(pad),y(pad),channels]
            zy = tf.pad(zy, paddings=paddings, mode="CONSTANT",
                        constant_values=0.)  # -> [batch, seq(pad), x(pad),y(pad),channels]

            # Apply the convolutions
            # note that the weights are shared totaly between the two views for the convolution part.
            xy = layer(xy)  # -> [batch, maxlengthSeq, maxlengthX,maxlengthY,channels]
            zy = layer(zy)  # -> [batch, maxlengthSeq, maxlengthX,maxlengthY,channels]

            # apply dropout
            if self.dropoutValueConv > 0:
                xy = self.dropoutsConv[idLayer](xy)
                zy = self.dropoutsConv[idLayer](zy)

            # apply maxpooling
            if self.doMaxPoolSpatial:
                xy = self.maxPoolSpatialLayerSame[idLayer](xy)
                zy = self.maxPoolSpatialLayerSame[idLayer](zy)
                # shapes are  [batch, maxlengthSeq, maxlengthX, maxlengthY, nbFeatureMap]
            ...
            if idLayer == 0:
                ...
                xy = self.addLayer([xy, toAddInFutureXY])  # xy = xy + toAddInFutureXY
                zy = self.addLayer([zy, toAddInFutureXZ])  # zy = zy + toAddInFutureXZ
                toAddInFutureXY = xy
                toAddInFutureXZ = zy
        ...
        if self.doGlobalAveragePooling:
            ...
            resConvXY = self.globalAveragePooling(xy)  # -> [batch, maxlengthSeq,nbFeatureMap]
            resConvXZ = self.globalAveragePooling(zy)  # -> [batch, maxlengthSeq,nbFeatureMap]
        else:
            ...
            resConvXY = tf.reshape(resConvXY,
                                   shape=[batch, maxlength, maxlengthX * maxlengthY[1] * self.nbFeatureMaps[-1]])
            resConvXZ = tf.reshape(resConvXZ,
                                   shape=[batch, maxlength, maxlengthX * maxlengthY[1] * self.nbFeatureMaps[-1]])

        # concatenate the features of the two views
        resClassif = tf.concat([resConvXY, resConvXZ], axis=-1)
